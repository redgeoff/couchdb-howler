  ...
  _createClass(Sockets, [{
    ...
    value: function close() {
      ...
      _sporks2.default.each(this._sockets, function (socket) {
        ...
        _this4.remove(socket.socket);
      });
    }
  }, {
    key: 'log',
    value: function log(socket, msg) {
      _log3.default.info({
        socketId: socket.id,
    ...
    key: 'emitChangeForDBName',
    value: function emitChangeForDBName(dbName) {
      var _this5 = this;

      var sockets = this.getByDBName(dbName);

      // Are there any subscribers to this DB?
      if (sockets) {
        _sporks2.default.each(sockets, function (socket) {
          _this5.log(socket, dbName + ' changed');
