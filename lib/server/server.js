'use strict';

var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }();

var _socket = require('socket.io');

var _socket2 = _interopRequireDefault(_socket);

var _log = require('./log');

var _log2 = _interopRequireDefault(_log);

var _notifier = require('./notifier');

var _notifier2 = _interopRequireDefault(_notifier);

var _utils = require('./utils');

var _utils2 = _interopRequireDefault(_utils);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

var Server = function () {
  function Server(opts) {
    _classCallCheck(this, Server);

    this._port = opts.port;
    this._io = (0, _socket2.default)();
    this._sockets = [];
    this._notifier = new _notifier2.default();
  }

  _createClass(Server, [{
    key: '_onConnection',
    value: function _onConnection() {
      var _this = this;

      this._io.on('connection', function (socket) {
        _this._logSocketInfo(socket, 'connection');

        _this._sockets.push(socket);
        _this._onSubscribe(socket);
        _this._onUnsubscribe(socket);
        _this._onDisconnect(socket);
      });
    }
  }, {
    key: '_logSocketInfo',
    value: function _logSocketInfo(socket, msg) {
      _log2.default.info({
        socketId: socket.id,
        remoteAddress: socket.conn.remoteAddress
      }, msg);
    }
  }, {
    key: '_onSubscribe',
    value: function _onSubscribe(socket) {
      var _this2 = this;

      socket.on('subscribe', function (dbName, callback) {
        _this2._logSocketInfo(socket, 'subscribe to dbName=' + dbName);

        _this2._notifier.subscribe(socket, dbName);

        _utils2.default.success(callback, { dbName: dbName });
      });
    }
  }, {
    key: '_onUnsubscribe',
    value: function _onUnsubscribe(socket) {
      var _this3 = this;

      socket.on('unsubscribe', function (dbName, callback) {
        _this3._logSocketInfo(socket, 'unsubscribe from dbName=' + dbName);

        _this3._notifier.unsubscribe(socket, dbName);

        _utils2.default.success(callback, { dbName: dbName });
      });
    }
  }, {
    key: '_onDisconnect',
    value: function _onDisconnect(socket) {
      var _this4 = this;

      socket.on('disconnect', function () {
        _this4._logSocketInfo(socket, 'disconnect');
        _this4._sockets.splice(_this4._sockets.indexOf(socket), 1);
      });
    }
  }, {
    key: 'start',
    value: function start() {
      this._onConnection();
      this._io.listen(this._port);
    }
  }, {
    key: 'stop',
    value: function stop() {
      // Stop accepting connections
      this._io.close();

      // Close each socket connection
      this._sockets.forEach(function (socket) {
        return socket.disconnect();
      });
    }
  }]);

  return Server;
}();

module.exports = Server;