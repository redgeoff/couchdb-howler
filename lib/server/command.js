...
var Command = function () {
  ...
  _createClass(Command, [{
    ...
    value: function _usage(argv) {
      this._argv = (0, _yargs2.default)(argv).usage('Usage: $0 [options]').example('$0 --couchdb-url https://admin:admin@localhost:5984 --port 3000').alias('u', 'couchdb-url').nargs('couchdb-url', 1).describe('couchdb-url', 'URL to the CouchDB cluster').alias('p', 'port').nargs('p', 1).describe('p', 'Port').default('p', 3000).alias('l', 'log-level').nargs('l', 1).describe('l', ['error|warn|info|debug. Setting a particular level implies that all log records at that', 'level and above are logged.'].join(' ')).default('l', 'info').help('h').alias('h', 'help').version('v').alias('v', 'version').demandOption(['couchdb-url']).epilog('Copyright 2017').argv;
    }
  }, {
    key: '_stop',
    value: function () {
